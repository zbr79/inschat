...
- **Full opencode-go switch (2026-08-29):** Gemini free tier retired app-wide. All chat/conclude/health/model/usage code now targets `https://opencode.ai/zen/go/v1` (opencode-go subscription key). Text chat: `deepseek-v4-pro` → `deepseek-v4-flash` fallback (pinnable via /models); images auto-route to `deepseek-v4-flash-vision-exp`; Conclude: `deepseek-v4-flash` → `deepseek-v4-pro` with `json_object` response format. `/models` and `/usage` rebuilt on the opencode-go catalog; official quota windows (`/zen/go/v1/usage`: rolling 5h / weekly / monthly) shown on both usage pages. `@google/genai` dependency removed; `lib/gemini.ts`/`lib/usage.ts` deleted; system-prompt logic moved to `lib/prompt.ts`, `ChatValidationError` to `lib/errors.ts`.
- [x] Voice dictation (2026-09-12): composer mic → 16 kHz WAV → `POST /api/transcribe` → shared local whisper.cpp sidecar on :9081. Guests allowed (60s cap). Nginx POST block required. Does not persist audio.
